"""
@brief:
A YouTube caption-fetching utility that retrieves transcript data (when available) for videos from specified YouTube channels.
It uses the YouTube Data API v3 to gather video metadata and the `youtube_transcript_api` to extract English captions.

Description:
--------
  This script defines a `Youtube` class that can be instantiated with an API key and output path, and can be called
  with a list of channel IDs to automatically fetch and save available captions to a `.txt` file or video URLs to a `.json` file.

Features:
--------
  - Fetch latest videos from one or more YouTube channel IDs
  - Retrieve captions (if available) using the YouTube Transcript API
  - Optionally save only video URLs to a `.json` file
  - Write fetched captions to a structured `.txt` file
  - Measure total runtime and display fetch metrics
  - Error logging for failed caption fetches

Example:
--------
```
  yt = Youtube(api_key="YOUR_API_KEY", filepath="./output/filename.txt", max_results=100, metrics=True)
  yt(channel_ids=["UC_x5XG1OV2P6uZZ5FSM9Ttw"])  # Fetch captions
  yt(channel_ids=["UC_x5XG1OV2P6uZZ5FSM9Ttw"], videoUrls=True)  # Just save video links
```
...
@dependencies
  ...
  - tqdm
  - json, os, timeit, logging

@author: @shivendrra
@src: https://github.com/shivendrra/web-graze/blob/main/graze/_transcripts.py
@license: MIT
