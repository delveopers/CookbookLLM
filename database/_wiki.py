"""
@brief:
A lightweight web scraper for extracting introductory paragraph content from Wikipedia based on user-provided queries.

This script builds URLs dynamically from queries, fetches their corresponding Wikipedia pages, extracts paragraph content,
and optionally scrapes internal linked pages found within the article. All scraped text is saved into a user-specified file.

Classes:
--------
- `Wikipedia`:
    The main interface class to run the scraper. Instantiate with an output filepath and call with a list of queries.

Functions:
----------
- `build_urls(query: str) -> str`:
    Constructs a Wikipedia URL for a given query.

- `scrapper(urls: str, headers: dict) -> tuple[list, int]`:
    Extracts all `<p>` tags (paragraphs) from a Wikipedia page.

- `fetch_extra_urls(query: str, headers: dict) -> list[str]`:
    Extracts internal Wikipedia article links from a main article page.

- `extra_scrape(url: str, headers: dict) -> tuple[list, int]`:
    Scrapes the internal link content similar to main page scraping.
...
scraper = Wikipedia(filepath="results/wiki_articles.txt", metrics=True)
scraper(["Machine Learning", "Black Hole"], extra_urls=True)
```
@author: @shivendrra
@src: https://github.com/shivendrra/web-graze/blob/main/graze/_wiki.py
@license: MIT
